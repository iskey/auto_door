...
def post_png_code_to_master(name):
    ...
    url= CMD_URL+"/article/status?name=%s&status=%s" % (name,"plz_fill_the_code@%s" % datetime.now().strftime("%H:%M:%S"))
    print url
...
def wait_for_submit_code(name):
    ...
    wait_time = [(1,2),(1,2),(2,3),(2,3),(2,3),(3,5),(3,5),(3,5),(5,10),(10,15),(1,2),(2,3),(2,3),(2,3),(3,5),(3,5),(3,5),(30,35),(1,2),(2,3),(2,3),(2,3),(3,5),(3,5),(3,5),(30,35),(1,2),(2,3),(2,3),(2,3),(3,5),(3,5),(3,5)]
    wait_flag = 0
    while True:
        ...
        #    return None
        if wait_flag < 60:
            random_s, random_e = (2, 3)
            t_sleep_time = random.randint(random_s, random_e)
            print "get submit code timer will sleep %s, Elapse:%s" % (t_sleep_time, wait_flag) , '\r',
...
def login_atm(name, user_id, user_passwd):
    ...
    while True:
        ...
        while touch_door_ok == False:
            ...
            print(yzm)
            if yzm is None:
                set_status(name,"touch_door_faield@%s" % datetime.now().strftime("%H:%M:%S"))
                return
            ...
            if is_touch_door_ok(response):
                print("touch door ok.")
                set_status(name,"touch_door_ok@%s" % datetime.now().strftime("%H:%M:%S"))
                touch_door_ok= True
                return
            else:
...
def auto_touch_door(user_name, user_id, user_passwd, m_time, n_time):
    ...
    while True:
        ...
        try:
            ...
            if sleep_time > (1):
                print "Autodoor trigger time: [ %s hours, %s minutes, %s seconds ]" %   \
                      (int(sleep_time / 3600), \
            ...
            #time.sleep(sleep_time)
            print ""
            print "Autodoor triggered."

            touchTime = datetime.now().replace(hour=hour, minute=minute, second=0)
            set_status(user_name,"client_hello@%s" % datetime.now().strftime("%H:%M:%S"))
            time.sleep(30)
